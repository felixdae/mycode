...
namespace comm
{
    namespace library
    {
        ...
        {
            public:
                Logger(const std::string& file_name, uint32_t max_file_num, uint32_t max_file_size, bool write_to_console);
                virtual ~Logger();
        ...
        {
            public:
                FunctionGuard(const std::string& func_name, std::shared_ptr<Logger>& logger);
                ~FunctionGuard();
            private:
