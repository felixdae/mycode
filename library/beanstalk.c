...
//  gettimeofday(&start, 0);
    ...
    if (BS_STATUS_IS(message->status, bs_resp_reserved)) {
        *result = job = (BSJ*)malloc(sizeof(BSJ));
        ...
        unsigned long job_size;
        sscanf(message->status + strlen(bs_resp_reserved) + 1, "%"PRId64" %lu", &job->id, &job_size);
        job->size = job_size;
...
int bs_peek_job(int fd, char *command, BSJ **result) {
    ...
    if (BS_STATUS_IS(message->status, bs_resp_found)) {
        *result = job = (BSJ*)malloc(sizeof(BSJ));
        ...
        unsigned long job_size;
        sscanf(message->status + strlen(bs_resp_found) + 1, "%"PRId64" %lu", &job->id, &job_size);
        job->size = job_size;
        job->data      = message->data;
        message->data  = 0;
        bs_free_message(message);
...
int bs_get_info(int fd, char *command, char **yaml) {
    ...
    if (BS_STATUS_IS(message->status, bs_resp_ok)) {
        unsigned long tsize;
        sscanf(message->status + strlen(bs_resp_ok) + 1, "%lu", &tsize);
        size = tsize;
        *yaml = message->data;
        (*yaml)[size] = 0;
        message->data = 0;
