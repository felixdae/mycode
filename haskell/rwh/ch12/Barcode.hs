...
testLOL = filter (\x->length x /= 7) leftOddList == []
...
testPL = filter (\x->length x /= 6) parityList == []
...
leftEncode '1' = {-(++"y") . -}(leftOddCodes !)
leftEncode '0' = {-(++"y") . -}(leftEvenCodes !)

rightEncode :: Int -> String
rightEncode = {-(++"x") . -}(rightCodes !)
...
-- row :: (Ix a) => a -> Array (a, a) c -> Array a c
...
findEAN13 pixmap = withRow center pixmap (fmap head . findMatch)
    where
        (_, (maxX, _)) = {-trace (show $ bounds pixmap) -}bounds pixmap
        center = (maxX + 1) `div` 2
